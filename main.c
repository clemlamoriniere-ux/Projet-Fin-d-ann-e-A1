...
trajectoire euler(planete planet){
    ...
    for (int i = 0; i < 365; i++){
        vector pos_t_plus_un = new_vector(addition(planet.r,multiplication(planet.v, dt)));
        vector v_t_plus_un = new_vector(addition(planet.v,multiplication(acceleration, dt)));
        traj.ensemble[i] = new_point(pos_t_plus_un, v_t_plus_un, i);
    }
    return traj;
}
